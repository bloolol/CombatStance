            ...
            switch (Main.TransitionAction)
            {
                ...
                case 2:
                    if (Game.Player.Character.IsUsingActionMod())
                    {
                        ...
                        Game.Player.Character.Task.PlayAnimation("missexile2", "enter_crouch_a", .55f, -4f, 600, AnimationFlags.StayInEndFrame, 0.0f);
...
/*
        private void Camera()
        {
            if (GameplayCamera.GetCamViewModeForContext(CamViewModeContext.OnFoot) != CamViewMode.FirstPerson)
            {
                if (Main.currentStance == Main.Stance.Crouching)
                {
                    ShoulderCameraSwitch.targetOffset = -.03f;
                    ShoulderCameraSwitch.TransitionCameraCrouch(Game.Player.Character);
                }
                else if (Main.currentStance == Stance.Prone)
                {

                }
                else if (Main.currentStance == Stance.Standing)
                {
                    //ShoulderCameraSwitch.targetOffset = 0f;
                    //ShoulderCameraSwitch.TransitionCameraCrouch(Game.Player.Character);
                }
            }
        }
*/
        private void Controller()
        {
            ...
            if (Game.IsControlJustPressed(GTA.Control.MultiplayerInfo) && Main.banButton)
                this.buttonPressTimer.Start();
            
            if (Main.currentStance == Main.Stance.Standing && Main.blockScopeCameraPlay)
            {
                if (Game.IsControlPressed(control) && Main.TransitionAction == 0)
                {
                   if (Main.TransitionAction == 0)
                        Main.TransitionAction = 2;
                }
                else if (Game.IsControlJustReleased(GTA.Control.MultiplayerInfo) && Main.TransitionAction == 0 && !Game.Player.Character.IsAiming)
                {
            ...
            else if (Main.currentStance == Main.Stance.Crouching && Main.TransitionAction == 0 && Main.blockScopeCameraPlay)
            {
                ...
                else if (Game.IsControlJustReleased(GTA.Control.MultiplayerInfo))
                {
                    if (!this.buttonPressTimer.IsRunning)
                        return;
                    if (this.buttonPressTimer.ElapsedMilliseconds >= 250L)
            ...
            else if (Main.currentStance == Main.Stance.Prone && Main.TransitionAction == 0 && Main.blockScopeCameraPlay)
            {
                ...
                else if (Game.IsControlJustReleased(GTA.Control.MultiplayerInfo) && !Game.Player.Character.IsAiming)
                {
                    if (!this.buttonPressTimer.IsRunning)
                        return;
        ...
        public static void SetStance(Main.Stance stance)
        {
            switch (stance)
            {
                ...
                case Main.Stance.Crouching:
                    ...
                    {
                        Function.Call(Hash.SET_PED_USING_ACTION_MODE, (InputArgument)(Entity)Game.Player.Character, (InputArgument)false, (InputArgument)(-1), (InputArgument)0);
                        Game.Player.Character.SetPedMovementClipset("move_ped_crouched", 0.55f);
